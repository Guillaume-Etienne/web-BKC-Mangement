...
function BookingWizard({ initial, clients, clientsLoading, rooms, accommodations, agencies, sources, houseRentals, roomRates, drivers, bookings, bookingRooms, taxiTrips, editingBookingId, isEditing, originEnquiry, recordedPaid, onCancel, onSave, lang }: WizardProps) {
  ...
  const selectedPickerClient = d.client_id ? clients.find(c => c.id === d.client_id) : null
            ...
            <div className="space-y-4">
              ...
              {!creatingClient ? (
                <>
                    ...
                    {clientsLoading ? (
                      ...
                      >
                        <div className="font-medium">
                          ...
                          {c.relationship_flag && (
                            <span className={`ml-2 px-1.5 py-0.5 rounded text-xs font-semibold ${relationshipFlagColors[c.relationship_flag]}`}>
                              {relationshipFlagIcons[c.relationship_flag]} {relationshipFlagLabels[c.relationship_flag]}
                            </span>
                          )}
                        </div>
                  ...
                  {/* Loud, not blocking: gui decides what to do with an "avoid"
                      client — this only makes sure he doesn't find out at check-in. */}
                  {selectedPickerClient?.relationship_flag === 'avoid' && (
                    <p className="text-sm text-red-700 dark:text-red-400 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900 rounded-lg px-3 py-2">
