    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${filter === 'all' ? 'bg-emerald-600 text-white' : 'bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:border-emerald-400 dark:hover:border-emerald-700'}`}>
          All time
        </button>
        {years.map(y => (
          <button key={y}
            onClick={() => setFilter(y)}
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${filter === y ? 'bg-emerald-600 text-white' : 'bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:border-emerald-400 dark:hover:border-emerald-700'}`}>
            {y}
          </button>
        ))}
      </div>
      ...
      {upcoming.length > 0 && (
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
          <div className="px-5 py-3 border-b bg-blue-50 dark:bg-blue-950/40">
            <h2 className="text-sm font-bold text-blue-800 dark:text-blue-400">Upcoming ({upcoming.length})</h2>
          </div>
      ...
      {past.length > 0 && (
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
          <div className="px-5 py-3 border-b bg-gray-50 dark:bg-gray-800">
            <h2 className="text-sm font-bold text-gray-700 dark:text-gray-300">Past ({past.length})</h2>
          </div>
        ...
        <tbody>
          {bookings.map(b => (
            <tr key={b.id} className="border-b hover:bg-gray-50 dark:hover:bg-gray-800">
              ...
              {showPrices && (
                <td className="px-4 py-2 text-right font-semibold text-emerald-800 dark:text-emerald-400 whitespace-nowrap">
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {(wePayBookings.length > 0 || wePaid > 0) && (
          <div className={`rounded-xl border px-5 py-4 ${weOwe > 0 ? 'bg-orange-50 dark:bg-orange-950/40 border-orange-200 dark:border-orange-900' : 'bg-green-50 dark:bg-green-950/40 border-green-200 dark:border-green-900'}`}>
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Center owes you</p>
        ...
        {(theyPayBookings.length > 0 || theyPaid > 0) && (
          <div className={`rounded-xl border px-5 py-4 ${theyOwe > 0 ? 'bg-blue-50 dark:bg-blue-950/40 border-blue-200 dark:border-blue-900' : 'bg-green-50 dark:bg-green-950/40 border-green-200 dark:border-green-900'}`}>
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">You owe us</p>
      ...
      {filteredBookings.length > 0 && (
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
          <div className="px-5 py-3 border-b bg-gray-50 dark:bg-gray-800">
            <h3 className="text-sm font-bold text-gray-700 dark:text-gray-300">Activity bookings ({filteredBookings.length})</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
            ...
            <table className="w-full text-sm">
              ...
              <tfoot>
                <tr className="bg-gray-100 dark:bg-gray-800 border-t-2 border-gray-300 dark:border-gray-700">
                  <td colSpan={2} className="px-4 py-2 text-right text-xs font-bold text-gray-600 dark:text-gray-400">
                    Paid to you
                  </td>
                  <td className="px-4 py-2 text-right font-bold text-green-800 dark:text-green-400">
                    {wePaid.toFixed(2)} €
                  </td>
                  <td />
                </tr>
                {theyPaid > 0 && (
                  <tr className="bg-gray-100 dark:bg-gray-800">
                    ...
                    <td className="px-4 py-2 text-right font-bold text-blue-800 dark:text-blue-400">
                      {theyPaid.toFixed(2)} €
                    </td>
                    <td />
                  </tr>
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="bg-gradient-to-r from-emerald-600 to-emerald-700 rounded-xl text-white px-6 py-5">
          <p className="text-emerald-200 dark:text-emerald-300 text-sm font-medium uppercase tracking-wide mb-1">{TYPE_LABEL} Provider</p>
          <h1 className="text-2xl font-bold">{provider.name}</h1>
          <div className="mt-2 flex flex-wrap gap-3 text-sm text-emerald-200 dark:text-emerald-300">
            ...
            {tabs.map(t => (
              <button key={t.key} onClick={() => setTab(t.key)}
                className={`px-5 py-2.5 font-medium text-sm transition-colors ${
                  tab === t.key
                    ? 'border-b-2 border-emerald-600 dark:border-emerald-500 text-emerald-700 dark:text-emerald-400'
