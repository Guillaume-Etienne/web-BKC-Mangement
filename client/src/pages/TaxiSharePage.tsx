        ...
        <div className="max-w-3xl mx-auto px-4 py-5 flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-blue-200 dark:text-blue-300">{tr.public_sub[lang]}</p>
            </div>
          </div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="flex flex-wrap gap-3 items-center">
          <select
            ...
            className="text-sm border rounded-lg px-3 py-2 bg-white dark:bg-gray-900 shadow-sm"
          >
          ...
          <div className="text-center py-16 text-gray-400 dark:text-gray-400">
            ...
            {!showPast && (
              <p className="text-sm mt-1">
                <button onClick={() => setShowPast(true)} className="text-blue-500 dark:text-blue-400 underline">
                  {tr.show_past[lang]}
                </button>
              </p>
            )}
            ...
            <div key={date}>
              ...
              <div className="space-y-2">
                {trips.map(trip => {
                  ...
                  return (
                    <div key={trip.id}
                      className={`bg-white dark:bg-gray-900 rounded-lg border shadow-sm p-4 ${
                        date < today ? 'opacity-60' : ''
                      }`}>
                      <div className="flex items-start justify-between gap-4">
                        {/* Left: time + type */}
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="text-xl font-bold text-gray-800 dark:text-gray-200 whitespace-nowrap">
                            {trip.start_time}
                          </div>
                          <div>
                            ...
                            {guest && (
                              <div className="text-xs text-gray-600 dark:text-gray-400 mt-0.5">👤 {guest}</div>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="text-right shrink-0">
                          {driver ? (
                            <div className="text-sm font-semibold text-blue-700 dark:text-blue-400">🚕 {driver.name}</div>
                          ) : (
                            <div className="text-sm font-semibold text-red-500 dark:text-red-400 italic">{tr.unassigned[lang]}</div>
                          )}
                      ...
                      {trip.notes && (
                        <div className="mt-2 pt-2 border-t text-xs text-gray-500 dark:text-gray-400 italic">
                          💬 {trip.notes}
                        </div>
                      )}
                    </div>
