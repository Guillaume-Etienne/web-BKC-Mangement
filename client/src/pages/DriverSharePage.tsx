      ...
      <div className="divide-y divide-gray-100 dark:divide-gray-800">
        {trips.map(t => (
          <div key={t.id} className="px-4 py-3">
            ...
            <div className="text-sm text-gray-600 dark:text-gray-400">{clientName(t)}</div>
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {trips.map(t => (
            <tr key={t.id} className="border-b hover:bg-gray-50 dark:hover:bg-gray-800">
              ...
              <td className="px-4 py-2 text-gray-700 dark:text-gray-300 font-medium">{clientName(t)}</td>
        ...
        <tfoot>
          <tr className="bg-gray-100 dark:bg-gray-800 border-t-2 border-gray-300 dark:border-gray-700 font-bold">
            <td colSpan={8} className="px-4 py-2 text-right text-gray-700 dark:text-gray-300 text-sm">{tr.total[lang]}</td>
            <td className="px-4 py-2 text-right text-amber-900 dark:text-amber-400">{mzn(total)}</td>
          </tr>
        </tfoot>
      </table>
    </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-blue-700 rounded-xl text-white px-6 py-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-blue-200 dark:text-blue-300 text-sm font-medium uppercase tracking-wide mb-1">{tr.driver_statement[lang]}</p>
              <h1 className="text-2xl font-bold">{driver.name}</h1>
              {driver.vehicle && <p className="text-blue-200 dark:text-blue-300 text-sm mt-1">{driver.vehicle}</p>}
              {driver.phone   && <p className="text-blue-200 dark:text-blue-300 text-sm">{driver.phone}</p>}
            </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
          <div className="px-5 py-3 border-b bg-amber-50 dark:bg-amber-950/40">
            <h2 className="text-sm font-bold text-amber-800 dark:text-amber-400">{tr.my_money[lang]}</h2>
          </div>
          <div className="px-5 py-4 space-y-2 text-gray-700 dark:text-gray-300">
            ...
            <p className="pt-2 border-t font-bold text-gray-900 dark:text-gray-100">
