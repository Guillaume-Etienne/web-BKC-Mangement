...
| relationship_flag | `'avoid' \| 'favorite' \| null` *(2026-09-11)* — la « black list » de gui : client à éviter / à retenir. Badge dans ClientsPage (liste, filtre, éditeur), avertissement dans le picker client de BookingsPage et dans `create_booking`/`create_booking_from_enquiry` (MCP). Le pourquoi va dans `client_notes`, pas ici — deux états seulement, pas un système de tags |
