...
|---|---|---|---|
| **`2026-08-14_enquiry_sources.sql`** | **Étape 0 du chantier Enquiries** (`ENQUIRIES.md`). Table `enquiry_sources` : les choix de « comment nous avez-vous trouvé ? », trilingues (jsonb `{fr,en,es}`), éditables dans **Options → 📣 Sources**. Semée avec 6 origines. Lecture anon limitée aux lignes **actives** et aux colonnes de libellé (le formulaire public en a besoin) ; aucune écriture anon. **4 vérifs en bas du fichier**, dont une **connectée** (le curl anon ne voit pas les lignes). ⚠️ Le code livré avec ajoute l'onglet Sources : sans la migration, l'onglet est vide en PROD. | ⬜ | ⬜ |
