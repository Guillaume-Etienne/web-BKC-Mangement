...
- **Une gagnée part en archive** mais dans **sa couleur**, et le tri/filtre par couleur la
  ramène immédiatement. L'archive n'est pas un cimetière : c'est là qu'on lit le taux de
  transformation et l'origine réelle des clients en fin de saison.
...
La chaîne Zapier ne meurt pas, elle change de source : demande enregistrée → **webhook base de
données** → Zapier → HubSpot + Mailchimp. C'est le mécanisme qui déclenche déjà
`notify-submission`. Appeler les API directement viendra plus tard, si gui veut se passer de
Zapier — on échangerait un abonnement contre des clés à maintenir.

- ⚠️ **Mailchimp est une liste de diffusion, pas un CRM.** Y verser quelqu'un qui demandait une
  ...
  (« tenez-moi informé »), stockée avec la demande ; **seuls les cochés** partent vers
  Mailchimp. C'est le seul ajout au formulaire — une case, pas un champ, donc quasi indolore.
  Si Mailchimp n'est pas prioritaire : rien à ajouter, et on n'envoie que vers HubSpot.
- **La synchro ne bloque jamais et reste visible** : la demande est enregistrée d'abord.
  Témoin sur la fiche + bouton « renvoyer ». Un Zap qui échoue en silence est un client perdu
  sans le savoir.
...
- Une demande gagnée quitte la liste de travail immédiatement, ou à la clôture de saison ?
- Faut-il normaliser « comment nous avez-vous trouvé ? » en catégories sur la fiche (pour
  compter) tout en gardant la réponse libre (pour la nuance) ?
